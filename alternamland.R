...
data <- merge(x=data, y=datasums, by.x="ggk", by.y="x")


data$alter <- factor(data$alter, levels=c("X.Unter.5.Jahre.", "X.5.bis.9.Jahre.", "X.10.bis.14.Jahre.", 
"X.15.bis.19.Jahre.",    
...
p <- ggplot(data = data, aes(x = alter, y = count, fill = gender)) +
  geom_line(data = subset(data, gender=="Männer" & ggk=="1"), 
           stat = "identity",
           position = "identity") +
  geom_line(data = subset(data, gender=="Frauen" & ggk=="1"),
           stat = "identity",
           position = "identity",
           mapping = aes(y = -count)) +
  geom_bar(data = subset(data, gender=="Männer"), aes(frame=ggk),
           stat = "identity",
           position = "identity") +
  geom_bar(data = subset(data, gender=="Frauen"), aes(frame=ggk),
           stat = "identity",
           position = "identity",
           mapping = aes(y = -count)) +
